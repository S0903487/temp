    ...
    <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-950/70 shadow-lg shadow-slate-950/20">
      <table className="min-w-full divide-y divide-slate-800 text-left text-sm">
        <thead className="bg-slate-900/90 text-slate-400">
          <tr>
            <th className="px-4 py-3 font-medium cursor-pointer hover:text-white transition select-none" onClick={() => onSort('fullName')}>
              Creator {renderSortIndicator('fullName')}
            </th>
            {isColVisible('platform') && <th className="px-4 py-3 font-medium">Platform</th>}
            {isColVisible('followers') && (
              <th className="px-4 py-3 font-medium cursor-pointer hover:text-white transition select-none" onClick={() => onSort('followers')}>
                Followers {renderSortIndicator('followers')}
              </th>
            )}
            {isColVisible('engagement') && (
              <th className="px-4 py-3 font-medium cursor-pointer hover:text-white transition select-none" onClick={() => onSort('engagementRate')}>
                Engagement {renderSortIndicator('engagementRate')}
              </th>
            )}
            {isColVisible('category') && (
              <th className="px-4 py-3 font-medium cursor-pointer hover:text-white transition select-none" onClick={() => onSort('category')}>
                Category {renderSortIndicator('category')}
              </th>
            )}
            {isColVisible('contact') && <th className="px-4 py-3 font-medium">Contact</th>}
            {isColVisible('status') && <th className="px-4 py-3 font-medium">Status</th>}
            {isColVisible('pipeline') && (
              <th className="px-4 py-3 font-medium cursor-pointer hover:text-white transition select-none" onClick={() => onSort('pipelineStatus')}>
        ...
        <tbody className="divide-y divide-slate-800 text-slate-300">
          {influencers.map((influencer) => {
            ...
            return (
              ...
              {isColVisible('category') && <td className="px-4 py-3"><span className="text-xs px-2.5 py-1 rounded-full bg-slate-900 border border-slate-800">{influencer.category}</span></td>}
              ...
              {isColVisible('status') && (
                <td className="px-4 py-3">
                  <span className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2.5 py-1 text-xs text-emerald-400 font-medium">
                    {influencer.status}
                  </span>
                </td>
              )}
