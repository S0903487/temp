...
export async function getSummary(_request: Request, env: Env, _auth: AuthedRequest): Promise<Response> {
  const [
    clientsCountRes,
    prospectCountRes,
    topClientsRes,
    ...
    env.DB.prepare(
      `SELECT cl.id, cl.name, cl.contact_email, cl.industry, cl.status
       FROM clients cl ORDER BY cl.created_at DESC LIMIT 10`
    ),
    ...
    env.DB.prepare(
      `SELECT i.id, i.full_name, i.username, i.followers, i.pipeline_status
       FROM influencers i ORDER BY i.created_at DESC LIMIT 10`
    ),
  ]);
